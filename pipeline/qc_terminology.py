...
def main() -> int:
    ...
    A("## 三、提示级变体（仅统计）")
    A("")
    A("这些变体只统计、不判违规。原因是本检查的一项**结构性局限**：")
    A("")
    A("> **校验器只看中文输出，无法知道某个中文词对应的是哪个英文词。**")
    A("")
    A("于是凡是「本术语的错误译法」同时又是「**邻近英文词的正确译法**」的词，"
      "都会被误判。实测中反复出现的例子：")
    A("")
    A("| 提示级变体 | 它其实对应的英文 | 属于哪个术语的禁用变体 |")
    A("|---|---|---|")
    A("| 代理 | proxy | Agent（智能体） |")
    A("| 控制台 | dashboard / console | Terminal（终端） |")
    A("| 轨迹 | trail | Trace（追踪记录） |")
    A("| 弱点 | weakness | Vulnerability（漏洞） |")
    A("| 开发周期 | development cycle | SDLC（软件开发生命周期） |")
    A("| 相关方 | party / parties | Stakeholder（干系人） |")
    A("| 数据泄露 | data leak | Data Exfiltration（数据外泄） |")
    A("| 度量 | measure（动词） | Metric（指标） |")
    A("")
    A("彻底解决需要把译文与源文按句对齐，再判断每个中文词对应哪个英文词——"
      "那会把校验器复杂化一个数量级，且对齐本身也会出错。")
    A("")
    A("**当前选择：接受一定误报，维持「人判断规则对不对、机器判断符合不符合规则」的分工。**")
    A("代价是硬性一致率这个数字只覆盖「无歧义错译」，"
      "读数时必须与下面的提示级命中合看。")
    A("")
    if soft_hits:
        A("| 术语 | 变体 | 篇目 | 次数 |")
