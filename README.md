...
| ⑦ 中文译稿产出 | ✅ **29/29 条** | 覆盖率 **85.3%**（分母 34），可用资料**全部**有中文；分块 271/271 |
...
**当前进度 29/29 条 = 85.3%**（**271/271 个分块全部完成**）——**可用的 29 条资料全部有中文译稿**。

> ⚠️ **一处更正（留在这里不删）**：交付前我一度声称"剩下 1 条是 0 字节的
> `lessons-from-ai-code-reviews`"——错了两次：那条属于"确认失效"，本就不在可用集里；
> 真正**可用但未译**的是 W7 的论文 `ai-assisted-code-review-assessment`
> （9 块 / 约 4.5 万字符）。该论文**现已补译完毕**，所以可用集是 29/29 全译。
...
| 若按大纲视频的既有处理移出分母 | 33 | 29/33 = 87.9% | ≥27 | ✅ 达标 |

**为什么采用保守口径**：把「抓不到的条目」移出分母会让数字变好看，而这正是本项目
刻意避免的事——5 篇失效条目一律留在分母里，当作对自己的惩罚（见 `拿来说明/03`）。
两个数字都给出来，结论在两种口径下都达标，读者可以自行核对。

### 关于挑战原文那句「≥ 80% 讲义/视频字幕」的口径说明

挑战的验收要点原文是「资料包覆盖课程主体内容（**≥ 80% 讲义/视频字幕**有对应中文）」。
本项目把分母定义为**大纲 readings 中指向本地文件的条目（34 条）**，理由与差异如下——
不解释清楚，读者按字面理解会得出完全不同的结论：

| 口径 | 分母 | 本仓库的结果 |
|---|---|---|
| **本仓库采用**：大纲里指向本地文件的 readings | 34 | **29/34 = 85.3% ✅** |
| 若按「讲义（Slides）」算 | 10 周讲义 | **0/10**——讲义全在 Google Slides/Drive，需 Google 账号，自动化不可行（已列缺口） |
| 若按「视频字幕」算 | 3 个视频 | **0/3**——视频无字幕，需额外获取步骤（已列缺口） |

**我的判断**：readings 是这门课真正的"阅读材料主体"（34 条里 31 篇 HTML + 3 份 PDF，
合计 ≈ 8.3 万词），也是唯一一条**能自动化、能复跑、能自证分母**的路径；
讲义与字幕当前既拿不到、也无法在管线里流转，把它们计入分母会让覆盖率变成
一个"受制于第三方权限"的数字（这也是外链不计入分母的同一条理由）。
**这个判断可能与你/平台的口径不同**——所以三个口径的数字全部列在表里，
且讲义与字幕的缺失在「已知缺口」中如实登记，不藏。
...
├── zh/                        中文译稿（29 条，85.3%）＋ README.md 索引
...
译稿在 `zh/` 下，**29 篇——29 篇可用资料全部译完**，文件名与英文底稿一一对应。
...
| 完整中文资料包 | `zh/`（**29/29 条可用资料 = 85.3%**，271/271 分块） | ✅ |
