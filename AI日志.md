...
| 2026-09-16 | 331 | 305 | 26 | 29 | 254,204 | 16:03:54 |
...
| 2026-09-16T16:03:54 | `ci_gate_depended_on_intermediate` | 新加的索引门禁（make_index.py --check）在 CI 上变红，本地却全绿；已改为不依赖未入库的中间产物 |
...
### `second_source_smoke_test`　2026-09-16T15:40:35

- **why**：挑战验收要点写的是「换一门课的资料，同样的流程能再次产出」——这句话不能靠声明；而且只在单源上跑，永远发现不了写死的默认值
...
### `ci_gate_depended_on_intermediate`　2026-09-16T16:03:54

- **why**：索引里的「分块数」来自 pipeline/work/chunks.json，而 pipeline/work/ 在 .gitignore 里、并未入库；CI 是全新 checkout，且 check 3 在 Ubuntu 上跑不清洗 PDF（无 macOS PDFKit），所以该文件里的 PDF 分块与本地不同 → 生成的索引与提交的索引不一致 → 门禁无故变红
- **how**：索引去掉「分块数」列（改在文件里说明运行期统计见 reports/coverage.md），只保留平台无关、且全部来自已入库内容的信息（中文标题、原文标题、译稿链接、中文字数）；并在无 .git、无 pipeline/work 的目录里模拟全新 checkout 逐个跑门禁验证
- **result**：全新 checkout 模拟：六项门禁全绿、确定性两遍一致；translate.py --status 在缺 chunks.json 时给出可读提示而非崩溃
- **cost**：索引少了一列信息（分块数），这是为了让门禁在别人机器上也成立而付的代价；已写进索引正文说明
